...
\author{Ram\'on D\'{\i}az-Uriarte$^{1}$, Daniel Rico$^{2}$, and Oscar M.\ Rueda$^{3}$}
...
% tableChromArray <- inputDataToADaCGHData(ff.or.RAM = "RAM",
...
%                      full.names = TRUE, pattern = "inputEx.RData")
% tableChromArray <- inputDataToADaCGHData(ff.or.RAM = "ff",
%                                          RDatafilename = fname)
...
\subsection{RAM objects and forking}


\label{allex-forking-local}
...
<<>>=
cbs.RAM.fork <- pSegmentDNAcopy(cgh.dat, chrom.dat)
...
hmm.RAM.fork <- pSegmentHMM(cgh.dat, chrom.dat, merging = "mergeLevels")
hmm.mad.RAM.fork <- pSegmentHMM(cgh.dat, chrom.dat, merging = "MAD")


hs.ml.RAM.fork <- pSegmentHaarSeg(cgh.dat, chrom.dat, 
                             merging = "mergeLevels")
...
biohmm.RAM.fork <- pSegmentBioHMM(cgh.dat,
                                chrom.dat,
                                pos.dat,
                                merging = "mergeLevels")
...
cghseg.merge.RAM.fork <- pSegmentCGHseg(cgh.dat,
                                          chrom.dat,
                                          merging = "mergeLevels")
...
\subsection{\textit{ff} objects and cluster}
\label{allex-ff-cluster}
...
cbs.ff.cluster <- pSegmentDNAcopy("cghData.RData", "chromData.RData", 
                                  typeParall = "cluster")
...
hmm.ff.cluster <- pSegmentHMM("cghData.RData", "chromData.RData", 
                              merging = "mergeLevels", 
                              typeParall = "cluster")
...
hs.ml.ff.cluster <- pSegmentHaarSeg("cghData.RData", "chromData.RData", 
                             merging = "mergeLevels", 
                                    typeParall = "cluster")
...
biohmm.ff.cluster <- pSegmentBioHMM("cghData.RData",
                                "chromData.RData",
                                "posData.RData",
                                merging = "mergeLevels", 
                                    typeParall = "cluster")
...
cghseg.merge.ff.cluster <- pSegmentCGHseg("cghData.RData",
                                          "chromData.RData",
                                          merging = "mergeLevels", 
                                          typeParall = "cluster")
...
waves.merge.ff.cluster <- pSegmentWavelets("cghData.RData",
                                           "chromData.RData", 
                                           merging = "mergeLevels", 
                                           typeParall = "cluster")
...
\subsection{\textit{ff} objects and forking}
\label{allex-ff-fork}
...
<<>>=
cbs.ff.fork <- pSegmentDNAcopy("cghData.RData", "chromData.RData", 
                                  typeParall = "fork")
...
hmm.ff.fork <- pSegmentHMM("cghData.RData", "chromData.RData", 
...
hs.ml.ff.fork <- pSegmentHaarSeg("cghData.RData", "chromData.RData", 
                             merging = "mergeLevels", typeParall = "fork")
...
biohmm.ff.fork <- pSegmentBioHMM("cghData.RData",
                                "chromData.RData",
                                "posData.RData",
                                merging = "mergeLevels", 
                                 typeParall = "fork")
...
cghseg.merge.ff.fork <- pSegmentCGHseg("cghData.RData",
                                          "chromData.RData",
                                          merging = "mergeLevels", 
                                       typeParall = "fork")
...
To make sure there are no file permission problems, we also explicitly
delete some of the "ff" files and objects (and we wait a few seconds to
allow pending I/O operations to happen before we delete the directory).
...
> 
> unix.time(tableChromArray <- inputDataToADaCGHData(ff.or.RAM = "ff",
