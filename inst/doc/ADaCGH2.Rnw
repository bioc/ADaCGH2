...
each project. All plot files and ff data will be stored there. At the end
of this example, we will remove this directory.
...
These are not strictly necessary, but we will explicitly stop the cluster
and remove the directory we created. To make sure there are no file permission 
problems, we also explicitly delete some of the "ff" files and objects (and we 
wait a few seconds to allow pending I/O operations to happen before
