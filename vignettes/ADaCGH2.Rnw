...
comparing the current version of ADaCGH2 (>= 2.2.1) with the former
...
analyze very large data sets in moderate hardware (see examples in the
``benchmark.pdf'' file).. Of course, what is ``lots'', ``many'', and
