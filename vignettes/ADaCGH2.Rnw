...
files, \texttt{inputToADaCGH} will probably fail, as it is set to read all
the files in the directory.}). Note that this will NOT work
under Windows, and thus we skip using \texttt{cutFile} under Windows, and
use a directory where we have stored the files split by column.
