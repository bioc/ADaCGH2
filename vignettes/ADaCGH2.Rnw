...
\item We show some benchmarking results with large data sets.
...
we allow parallelization of analysis (and data reading) using forking. 


% , to use instead local R objects
% plus forking, which will likely be the preferred usage mode when plenty of
% RAM is available in a single multicore machine.
...
\item[RAM objects] The ``usual'' R objects (in our case, mainly data
  ...
  objects. Technically, a given data frame, for instance, need to be in
...
\begin{center}
  ...
  \begin{tabular}{>{\centering}p{3.1cm}p{0.01cm}>{\centering}p{3.7cm}p{0.01cm}>{\centering}p{3.2cm}p{0.001cm}}
    ...
    \hline
    \centerline{Single node,} many cores/node &&\centerline{RData objects (?), forking}
...
speed of communication between nodes, etc.

For large data sets, the recommended usage involves always using
\textit{ff} objects. Using \textit{ff} objects is slightly more
cumbersome, but can allow us to analyze very large data sets in moderate
hardware (see examples in section \ref{very-large-example}).
...
%                       &  Lots of RAM                &  Little RAM  \\
% \hline
%  Many cores per node  &  No ff, fork                &  ff, fork    \\
%  and few nodes        &  (but make ff optional)     &              \\
%                       &  (but make cluster optional)    &              \\
%  Few cores per node   &  ff, cluster                    &  ff, cluster     \\
%  and many nodes       &  (we need acess to object   &              \\
%                       &  from several nodes, so we  &              \\
%                       &  use ff.                    &              \\
% \end{tabular}
% \end{center}

Of course, what is ``lots'', ``many'', and ``large'', will depend on the
arrays you analyze and the hardware.
  
  
%%% FIXME: fuse previous table and next section, and discuss reasons for choices.  
...
less cumbersome to deal with than \textit{ff} objects. But see section
\ref{very-large-example} for other comments.
...
We will use \texttt{inputDataToADaCGHData} to produce the three objects
...
inputDataToADaCGHData(ff.or.RAM = "RAM",
...
frame. This way, we use \texttt{inputDataToADaCGHData} basically for its
...
\paragraph{Skipping the call to \texttt{inputDataToADaCGHData}}
Since our data are already available as an R data frame, and if we are not
interested in the checks provided by \texttt{inputDataToADaCGHData}, we do
...
tmp <- inputDataToADaCGHData(ff.or.RAM = "RAM",
...
The function \texttt{inputDataToADaCGHData} will work with both compressed
...
\texttt{inputDataToADaCGHData}, if they produce \textit{ff} output (i.e.,
...
keep reusing \texttt{inputDataToADaCGHData}, but note that in all the
...
fnameRdata <- list.files(path = system.file("data", package = "ADaCGH2"),
                     full.names = TRUE, pattern = "inputEx.RData")
inputDataToADaCGHData(ff.or.RAM = "ff",
                      RDatafilename = fnameRdata)
@ 
...
data into R. Even if we remove the original data and call \texttt{gc()}, R
might not return all of the memory to the operating system, and this might
be inconvenient in multiuser environments and/or long running
processes. Thus, it is possible to execute the above in a separate R
process that is spawned exclusively just for the conversion. For instance,
we could use the \texttt{mcparallel} function and do:

<<eval=FALSE>>=
mcparallel(inputDataToADaCGHData(ff.or.RAM = "ff",
                                 RDatafilename = fnameRData),
                                 silent = FALSE)
...
conversion to \textit{ff} carried out) dies after the conversion, freeing
...
\subsubsection{Reading data from a directory}
\label{reading-dir-1}
See section \ref{reading-dir} for further details.  This option is a
reasonable option with very large data sets. The initial data reading will
use forking and, once we have saved the objects as \textit{ff} objects, we
can apply all the subsequent analysis steps discussed in the rest of this section.
...
Unless we use forking, we need to set up and initialize a cluster. Cluster
initialization uses the functions provided in \texttt{parallel}. In the
example we will use a sockect cluster, but MPI can also be used (that is
what we use in our servers). In this example we will use a four node
cluster.
...
generator. The third expression sets the cluster just created as the
default cluster. This is important: to simplify function calls, we do not
pass the cluster name around, but rather expect a default cluster to be
set up. And, of course, we need to make the package available in all the
nodes of the cluster.


The last three lines make sure the slave or child process use the same
directory as the master. Because we created the cluster after changing
directories (section \ref{choosewd}) this step is not really needed
here. But we make it explicit so as to verify it works, and as a reminder
that you will need to do this if you change directories AFTER creating the
cluster. If you run on a multinode cluster, you must ensure that the same
directory exists in all machines. (In this case, we are running on the
localhost).
...
to store all plot files and ff data. 


Since we already did that above (section \ref{choosewd}) we do not repeat
it here. However, for real work, you might want to keep different analyses
associated to different working directories.
...
Even if we are using forking, we might still want to carry it the
conversion to \textit{ff} objects in a separate process, as in section
\ref{separate-proc} since the conversion to \textit{ff} objects might be
the step that consumes most RAM in the whole process.
...
parallalized, which allows us to speed up the reading process
significantly (the parallelization uses forking, and thus you will see no
speed gains in Windows). Finally, we use \textit{ff} objects for
intermediate storage.

\paragraph{Cutting the original file into one-column files}
We provide a simple function, \texttt{cutFile}, to do this job. For instance

<<>>=
fnametxt <- list.files(path = system.file("data", package = "ADaCGH2"),
                         full.names = TRUE, pattern = "inputEx.txt")
dir.create("cuttedFileAndffs")
setwd("cuttedFileAndffs")
cutFile(fnametxt, 1, 2, 3)
cuttedFileAndffs.dir <- getwd()
setwd("../")
@ 

We create a new directory and carry out the conversions there since the
upper level directory is already populated with other files we have been
creating. If we cut the file here, we would later need to specify a
lengthy list of files to exclude in the arguments to
\texttt{inputDataToADaCGHData}. To avoid that, we create a directory, and
leave the files there. After cutting, we return to the former level
directory, to keep that directory with only the files for input.


\texttt{cutFile} will run several jobs in parallel to speed up the cutting
process, and will produce files with the required naming conventions of
\texttt{inputDataToADaCGHData}. Note that \texttt{cutFile} is unlikely to work
under Windows. 
...
\texttt{cut} (which is also used internally by \texttt{cutFile}) to
produce one-column files from a file called \texttt{Data.txt}, with 77
arrays/subjects, where cutting the data part is parallelized over four
processors:
...
contraints); see section \ref{very-large-example}.


%% <<>>=
%% namepath <- system.file("example-datadir", package = "ADaCGH2")
%% inputDataToADaCGHData(ff.or.local = "ff",
%%                       path = namepath)
%% @ 
...
array. Always use the column names to identify unambiguously the data and
the results.



\paragraph{What about performing this step in a separate process?}
In section \ref{separate-proc} we performed the data preparation in a
separate process, to free up RAM to the OS right after the conversion. You
can do that too here if you want, but we have not found that necessary,
since the memory consumption when reading column by column is often
small. See examples with large data sets in section \ref{large-ex-read}.
...
<<>>=
tmp <- inputDataToADaCGHData(MAList = MA, 
                             robjnames = c("cgh-ma.dat", "chrom-ma.dat",
                                          "pos-ma.dat", "probenames-ma.dat"))

tmp <- inputDataToADaCGHData(MAList = MA2, 
                             robjnames = c("cgh-ma.dat", "chrom-ma.dat",
                                          "pos-ma.dat", "probenames-ma.dat"),
                             minNumPerChrom = 4)
@ 
...
\section{A (non-runnable) example with a large data set and a comparison
  of approaches}
\label{very-large-example}


We discuss here a specific example to give an rough idea of timings and
possible preferred approaches for analyzing very large data sets.

\subsection{Data set and hardware}
\label{sec:data-set-hardware}

The simulated data set contains 6,067,433 rows and 1000 columns of data. In
some examples shown below, we use smaller subsets of the same data. There
are 421000 missing values per data column. The ASCII file with the data is
about 96 GB and the RData for those same data is 46 GB (without
compression). We will analyze the full data set with 1000 columns, and
...
has 16 cores, each node has 64 cores. Each node has 256 GB RAM. Both nodes
are connected by Infiniband (40Gb/s).  For the data presented here, when
using a single node, the data live on an xfs partition of a RAID0 array
of SCSI disks (15000 rpm) that are local to the node doing the
computations. When using the two nodes, the data live on that same local
SCSI drive, which is seen by the other node using a simple NFS setup (we
have also used distributed file systems, such as FhGFS, but they have
tended to be a lot slower in these experiments; your mileage might
vary). Therefore, in the table entries below, executions using both nodes
will indicate ``124 cores'' \footnote{124 is not a typo; it is 124, even
  if the total number of cores is $128 = 64 * 2$. This is due to the
  following documented issue with MPI and Infiniband:
  ...
  a chance to correct this problem yet. Regardless, it would be a
  difference of 4 process out of 124.}
...
tables, this is indicated as ``12, Coleonyx''.
...
version R-2.15.1 as available from the Debian repository (v.\
2.15.1-5). The Xeon workstation was running R-2.15.1, patched version
(2012-10-02 r60861), compiled from sources.
...
\item[Max.\ $\Sigma$ mem.] A simple attempt to measure of the sum of all the
  memory used by all the process. Right before starting the execution of
  our function, we call the user command \texttt{free} and record the
  value reported by the ``-/+ buffers/cache'' row. Then, while the
  function is executing, we record, every 0.5 seconds, that same
  ...
  \item[Fork/MPI] Whether forking (via \texttt{mclapply}) or MPI are used
...
\item[Cores] Number of cores, and machine. 64: 64 cores in a single node
  ...
  \item[ff/in-RAM] If the data for the analysis was stored as an
    \textit{ff} object, or as an RData data frame that was loaded in full
    before the analysis.
...
\begin{threeparttable}
  ...
  \begin{tabular}{p{3cm}p{1.8cm}p{1.8cm}p{2.5cm}p{2.5cm}}
 
    \hline
    &Columns & Wall time (min.)& Max.\ mem. (\texttt{gc}), Gb &
    Max.\ $\Sigma$ mem., Gb\\
\hline
    Txt file to \textit{ff} & 1000 & 2630 & 1.3 & NA (not calculated) \\
    Directory to \textit{ff} & 1000 & 29 & 1.3 & 41.21 \\
    Directory to \textit{ff} (Coleonyx, 12 cores) & 1000 & 88 & 1.3 & 8.4 \\
    RData to \textit{ff} & 1000 & 29.6   &   169 & 168.3 \\
    Directory to in-RAM & 1000 & 22 + 2\tnote{a} & 96 & NA. Output unusable for analysis \\
    RData to in-RAM & 1000 & 22 + 2\tnote{b} &  139 & NA. Output unusable for analysis \\

Directory to in-RAM & 200 & 7.7 + 0.4\tnote{d} & 20 & 38 \\
Directory to \textit{ff} & 200 & 3.6 & 1.3 & 38 \\

Directory to \textit{ff} & 50 & 3.2 & 1.3 & 28 \\
Directory to in-RAM & 50 & 5.8 + 0.1\tnote{c} & 5.8 & 27 \\
...
\hline
  ...
  \begin{tablenotes}
    \item[a] The 2 reflects the time needed to save the resulting RData
    \item[b] The 2 reflects the time needed to save the resulting RData
    \item[c] The 0.1 reflects the time  needed to save the resulting RData
    \item[d] The 0.4 reflects the time  needed to save the resulting RData
  \end{tablenotes}
\end{threeparttable}
\end{center}
...
\begin{threeparttable}
  ...
  \begin{tabular}{p{1.5cm}p{1.2cm}p{1.2cm}p{1.2cm}p{1.5cm}p{1.8cm}p{1.8cm}p{1.6cm}}
    
    \hline

Method& Fork /MPI & Cores & ff/in-RAM & Columns & Wall time (min.) & Max.\ mem. (\texttt{gc}), Gb & Max.\ $\Sigma$ mem., Gb\\
...
HaarSeg & Fork &64  & in-RAM & 50 & 0.7 + 2.5 + 0.850 \tnote{a}& 14.4 & 140\\
HaarSeg & MPI &64  & ff & 50 & 0.63 & 0.14 & 24.6\\
&&&&&&&\\
HaarSeg & Fork &64  & in-RAM & 1000 & NA & NA & \textbf{Cannot allocate memory} \\
HaarSeg & Fork &64  & in-RAM & 200 & NA & NA & \textbf{Cannot allocate memory}\\
\hline
   
  \end{tabular}

  \begin{tablenotes}
    \item[a] 0.7 + 2.5 + 0.850: load data, analyze, and save results.
  \end{tablenotes}
...
\item When using two nodes (i.e., almost doubling the number of cores),
  ...
  lies in access to files, which live on a SCSI disk which is accessed via
  NFS. With other hardware/software configurations, access to shared files
  might be much faster. Regardless, the MPI costs might not be offset if
...
\item When using \textit{ff}, the exact same operations in systems with
  ...
  directory of when forking, with a 1000 column data set. You can tune
  parameters when you load the \textbf{ff} package, but even if you don't
  (and, by default, we don't), defaults are often sensible and will play
  in your favor.
...
\item Even for relatively small examples, using \textit{ff} can be faster
  ...
  repeated cache misses when the complete data frame does not fit in the
  cache(s).
...
> 
> unix.time(tableChromArray <- inputDataToADaCGHData(ff.or.local = "ff",
...
Calls: unix.time ... inputDataToADaCGHData -> is.na -> is.na.data.frame -> do.call -> cbind
