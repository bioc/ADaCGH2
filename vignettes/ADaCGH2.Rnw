...
analyze very large data sets in moderate hardware (see examples in section
\ref{very-large-example}). Of course, what is ``lots'', ``many'', and
``large'', will depend on the arrays you analyze and the hardware.
...
We provide a simple function, \texttt{cutFile}, to do this job. For
instance (note that this will NOT work under Windows, and thus we skip
using \texttt{cutFile} under Windows, and use a directory where we have
...
the RData for those same data is 46 GB (without compression; 41 GB with
the standard R compression). We will analyze the full data set with 1000
columns, and some smaller versions of that data set (with 200 and 50 data
columns).
...
load and uses a total of 46 GB (45.7 as reported by \texttt{gc}, and
adding Ncells and Vcells, or 45.6 as reported by
...
instead of being a matrix; a similarly sized matrix with just numeric data
would be about 45.3 GB\footnote{A submatrix of those same data with 100
