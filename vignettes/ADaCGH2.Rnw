...
\item[Max.\ $\Sigma$ mem.] A simple attempt to measure the memory used by
  ...
  \item[Fork/MPI] Whether forking (via \texttt{mclapply}) or MPI (using
    ...
    packages\footnote{The packages are DNAcopy, as available from
      ...
      \url{https://r-forge.r-project.org/projects/haarseg/}; \cite{haar}}.
...
\begin{threeparttable}
  ...
  \caption{Time and memory usage with default options}    
...
CBS&Fork&12&ff&1000&1181.1 & 0.142 & xx \\
...
\begin{threeparttable}
  ...
  \caption{Time and memory usage without merging and comparison with
    non-parallized executions. These examples have all been run on the
    Dell Power Edges.}    
  \begin{tabular}{p{1.5cm}p{1.2cm}p{1.2cm}p{1.2cm}p{1.5cm}p{3.0cm}p{1.8cm}p{1.6cm}}
...
% CBS & Fork &10  & ff & 1000 &xxxx & xxxxx & xxx \\
