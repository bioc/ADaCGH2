...
\begin{threeparttable}
  ...
  \begin{tabular}{p{1.5cm}p{1.2cm}p{1.2cm}p{1.2cm}p{1.5cm}p{1.95cm}p{1.8cm}p{1.6cm}}
...
HaarSeg & NP &-  &RAM  & 100 &  1.5 + 22.9 + 1.4\tnote{b}   & 12.5 & 12.5\tnote{d}  \\
CBS     & NP &-  &RAM  & 100 & x   &   x   &  x\tnote{d}  \\
...
\hline
  ...
  \begin{tablenotes}
    ...
    \item[a] With option \texttt{merging = "none"} to make it comparable
     ...
     \item[b] 1.5 + 22.9 + 1.4: load data, analyze, and save results. Since
      ...
      If there are
      no missing values in this data set, the total time of analysis
      (i.e., sending the whole matrix at once and not checking nor
      removing NAs) is 3.3 minutes.
    \item[c] xx + xx + xx: load data, analyze, and save results.
