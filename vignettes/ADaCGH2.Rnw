...
\item Optional storage of, and access to, data using the \textit{ff}
  package (\url{https://CRAN.r-project.org/package=ff}),
...
\item Parallelization and \textit{ff} can be used simultaneously. WaviCGH
  ...
  \textit{ff} on a computing cluster with shared storage over NFS. (Note: the site is no longer available).
...
<<input-ff-separate-proc, eval=FALSE>>=
...
<<input-ff-separate-proc-ex3, eval=FALSE>>=
mcparallel(inputToADaCGH(ff.or.RAM = "ff",
                                 RDatafilename = fnameRdata),
           silent = FALSE)
