...
function (from package \textbf{paralle}) and do:
...
fact, that is what we use in our servers). In this example we will use a
four node cluster.
...
set up. The fourth line makes the \textbf{ADaCGH2} package available in
...
\subsubsection{Cutting the original file into one-column files}
...
\item Call \texttt{inputDataToADaCGHData}: 
  \texttt{inputDataToADaCGHData(ff.or.RAM = "ff", path = getwd(),
    excludefile = "afile.txt")}
\end{enumerate}

(In this vignette the work flow was not as easy because we are running
lots of different examples, with several different work flows.)
...
\texttt{inputDataToADaCGHData}. Note that \texttt{cutFile} is unlikely to
work under Windows.
...
\texttt{cut} under bash (which is similar to what we do internally in
...
We use a simulated data set that contains 6,067,433 rows and 1000 columns
of data; these are, thus, data for an array with 6 million probes and data
for 1000 subjects. In some examples shown below, we use smaller subsets of
the same data (with only 200 or 50 columns). There are 421,000 missing
values per data column. The ASCII file with the data is about 96 GB and
the RData for those same data is 46 GB (without compression). We will
analyze the full data set with 1000 columns, and some smaller versions of
that data set (with 200 and 50 data columns).
...
using a single node, the data live on an xfs partition of a RAID0 array of
SAS disks (15000 rpm) that are local to the node doing the
computations. When using the two nodes, the data live on that same local
SAS drive, which is seen by the other node using a simple NFS setup (we
...
(2012-10-02 r60861), compiled from sources. Open MPI is version 1.4.3-2 (as
...
\item[Cores] Number of cores, and machine. 64: 64 cores in a single node
  ...
  \item[ff/in-RAM] If the data for the analysis was stored as an
    \textit{ff} object, or as an RData data frame that was loaded 
    before the analysis.
...
\begin{threeparttable}
  ...
  \begin{tabular}{p{3cm}p{1.8cm}p{1.8cm}p{2.65cm}p{2.65cm}}
